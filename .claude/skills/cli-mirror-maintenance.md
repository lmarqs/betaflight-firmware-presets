...
| `docs/cli/bf-4.4-cli-commands.md` | `betaflight/betaflight@4.4-maintenance` | source-derived (no wiki page); body is a hand-curated table of the lmarqs preset surface |
...
2. **Spot-check a handful of parameters** the user cares about (or
   parameters used by lmarqs presets — `roll_rc_rate`, `osd_warn_bitmask`,
...
2. **Decide body source**:
   ...
   - If no wiki page, build a source-derived table covering at minimum
     the lmarqs preset surface (use `bf-4.4-cli-commands.md` as the
     model).
