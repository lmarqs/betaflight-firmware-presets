...
**IMPORTANT:** This fork (branch `lmarqs-custom-presets`) maintains **only the MARX5 presets** (`marx5_rates.txt`, `marx5_modes.txt`, `marx5_osd.txt`) under `presets/2025.12/{rates,modes,osd}/`. The other preset directories are kept in sync with upstream but are not edited here. Limit changes, validations, and indexing work to MARX5 files unless explicitly asked otherwise.
...
Generated files (not committed): `index.json`, `index_hash.txt`, `public/`
...
- One preset per PR
- **IMPORTANT:** Never commit `index.json` or `index_hash.txt` (generated artifacts)
