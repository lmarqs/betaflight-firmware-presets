# Betaflight 4.0 CLI Reference (legacy)

- **Status**: archive — provided for legacy preset compatibility checks; do NOT use as the primary target for new presets
- **Authoritative source** (overrides this mirror on conflict): [`betaflight/betaflight@4.0-maintenance`](https://github.com/betaflight/betaflight/tree/4.0-maintenance/src/main/cli)
- **Wiki mirror**: <https://betaflight.com/docs/wiki/guides/archive/Betaflight-4.0-CLI-commands>
...
gh api -H "Accept: application/vnd.github.raw" \
  repos/betaflight/betaflight/contents/src/main/cli/settings.c?ref=4.0-maintenance \
  > /tmp/bf-4.0-settings.c
gh api -H "Accept: application/vnd.github.raw" \
  repos/betaflight/betaflight/contents/src/main/fc/parameter_names.h?ref=4.0-maintenance \
  > /tmp/bf-4.0-pnames.h
```
...
- Range/enum from source: `grep '"<param_name>"' /tmp/bf-4.0-settings.c` — row contains `.config.minmaxUnsigned = { lo, hi }` or `.config.lookup = { TABLE_<NAME> }` (enum table earlier in same file as `lookupTable<Name>`).
- `PARAM_NAME_*` macros: `grep '#define PARAM_NAME_<UPPER> ' /tmp/bf-4.0-pnames.h`.

## Compat notes

- Legacy version. Many parameters since renamed or removed in 4.4+. Cross-check against current mirror before reusing.
