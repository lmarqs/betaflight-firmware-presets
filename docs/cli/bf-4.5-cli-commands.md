# Betaflight 4.5 CLI Reference

- **Status**: active (current stable)
- **Authoritative source** (overrides this mirror on conflict): [`betaflight/betaflight@4.5-maintenance`](https://github.com/betaflight/betaflight/tree/4.5-maintenance/src/main/cli)
- **Wiki mirror**: <https://betaflight.com/docs/wiki/guides/archive/Betaflight-4.5-CLI-commands>
...
gh api -H "Accept: application/vnd.github.raw" \
  repos/betaflight/betaflight/contents/src/main/cli/settings.c?ref=4.5-maintenance \
  > /tmp/bf-4.5-settings.c
gh api -H "Accept: application/vnd.github.raw" \
  repos/betaflight/betaflight/contents/src/main/fc/parameter_names.h?ref=4.5-maintenance \
  > /tmp/bf-4.5-pnames.h
```
...
- Range/enum from source: `grep '"<param_name>"' /tmp/bf-4.5-settings.c` — row contains `.config.minmaxUnsigned = { lo, hi }` or `.config.lookup = { TABLE_<NAME> }` (enum table earlier in same file as `lookupTable<Name>`).
- `PARAM_NAME_*` macros: `grep '#define PARAM_NAME_<UPPER> ' /tmp/bf-4.5-pnames.h`.
