...
- **Authoritative source** (overrides this mirror on conflict): [`betaflight/betaflight@2025.12-maintenance`](https://github.com/betaflight/betaflight/tree/2025.12-maintenance/src/main/cli)
- **Wiki mirror**: <https://betaflight.com/docs/wiki/guides/current/Betaflight-2025.12-CLI-commands>
...
gh api -H "Accept: application/vnd.github.raw" \
  repos/betaflight/betaflight/contents/src/main/cli/settings.c?ref=2025.12-maintenance \
  > /tmp/bf-2025.12-settings.c
gh api -H "Accept: application/vnd.github.raw" \
  repos/betaflight/betaflight/contents/src/main/fc/parameter_names.h?ref=2025.12-maintenance \
  > /tmp/bf-2025.12-pnames.h
```

## Lookup

- Default in this mirror: `grep '^<param_name>' <this-file>` for parameter default; section headers (`## ...`) for command syntax.
- Range/enum from source: `grep '"<param_name>"' /tmp/bf-2025.12-settings.c` — row contains `.config.minmaxUnsigned = { lo, hi }` or `.config.lookup = { TABLE_<NAME> }` (enum table earlier in same file as `lookupTable<Name>`).
- `PARAM_NAME_*` macros: `grep '#define PARAM_NAME_<UPPER> ' /tmp/bf-2025.12-pnames.h`.

## Compat notes

- `osd_canvas_width` / `osd_canvas_height` are gated behind `#ifdef OSD_CANVAS_SIZE_DEBUG` and **not** exposed in production 2025.12 builds, even though they appear in `settings.c`.
